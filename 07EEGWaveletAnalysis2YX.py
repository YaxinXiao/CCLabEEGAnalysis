...
# ==== USER PARAMETERS ====
# =========================
band_idx = {'theta': [0, 4], 'alpha': [4, 9], 'beta': [9, 26], 'gamma': [26, 46]}
...
plt.savefig('figure/TimeFreq_Group_Diff_FzCzPz.png')
plt.close()
...
fig.tight_layout()
plt.savefig("figure/Significant_Vocal_vs_Mean.png")
plt.close()
...
for group in ['euro', 'asia']:
    ...
    plt.savefig(f'figure/Alpha_ERP_{group}_FzCzPz.png')
    plt.close()
...
for c in ['euro', 'asia']:
    ...
    for band, (start, end) in band_idx.items():
        ...
        outname = f'results/wavelet/{band.title()}_Att_VS_{c}_5C.png'
